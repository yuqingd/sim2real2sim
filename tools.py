...
def simulate_real(agent, envs, steps=0, episodes=0, state=None):
  # Initialize or unpack simulation state.
  if state is None:
    step, episode = 0, 0
    done = np.ones(len(envs), np.bool)
  ...
  while (steps and step < steps) or (episodes and episode < episodes):
    ...
    #predict sim params
    pred_sim_params = agent.predict_sim_params(obs, action)
...
def load_episodes(directory, rescan, length=None, balance=False, seed=0, real_world_prob=None):
  ...
  while True:
    ...
    if real_world_prob and "real_world" in cache[keys[0]]:
