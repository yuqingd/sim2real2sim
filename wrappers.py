...
class RealWorldRL:
  ...
  def __init__(self, name, size=(64, 64), camera=None, real_world=False, sparse_reward=True, dr=None, use_state=False,
    ...
    task = 'realworld_' + task

    if isinstance(domain, str):
      import environments.realworldrl_suite.realworldrl_suite.environments as rwrl
...
class Kitchen:
  def __init__(self, task='reach_kettle', size=(64, 64), real_world=False, dr=None, mean_only=False, anneal_range_scale=0, predict_val=False,
  ...
  def setup_task(self):
    ...
    if 'push' in self.task or 'slide' in self.task:
      ...
      box_loc[2] += np.random.normal(0, .075) #move box height only
