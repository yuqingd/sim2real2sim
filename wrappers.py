...
class Kitchen:
  ...
  def step(self, action):
    ...
    ikresult = qpos_from_site_pose(physics, self.end_effector_name, target_pos=xyz_pos, joint_names=joint_names, max_steps=20)  # TODO: possibly specify which joints to move to reach this??
    qpos = ikresult.qpos
    ...
    if not success:
      reward = reward * 2
    done = np.abs(reward) < 0.1   # TODO: tune threshold
    info = {}
    obs = {}
    if self.use_state:
