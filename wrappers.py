...
class Kitchen:
  def __init__(self, task='reach_kettle', size=(64, 64), real_world=False, dr=None, use_state=False, step_repeat=1, step_size=0.001, use_gripper=False): #  TODO: are these defaults reasonable? It's higher than than the pybullet one for now, but just for testing.
  ...
  def step(self, action):
    ...
    obs['state'] = self.goal
    if self.use_state:
      obs['state'] = np.concatenate([obs['state'], self._env.data.ctrl[:self.arm_njnts]])
      if self.use_gripper:
