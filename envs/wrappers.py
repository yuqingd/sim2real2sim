...
class L2Low(gym.Wrapper):
    ...
    def __init__(self, env, delta_max=None, # delta max is a list with one arg per skill dim.
                            epsilon=0.25, 
                            sparse_reward=25, 
                            reward_scale=0.1,
        ...
        # if not self.additive_goals:
        #     assert not goal_range_high is None and not goal_range_low is None
        #     self.goal_range_low = np.array(goal_range_low)
        #     self.goal_range_high = np.array(goal_range_high)
        ...
        # If we use HER, we can't use relative in the wrapper. Although, we should accept the parameter.
        if self.use_her:
            self.relative = False
        else:
            self.relative = relative
        self.last_env_obs = None # NOTE: May have to alter to reset here so we get last_env_obs if they call step before reset
        self.goal = None # May need to set
        self.env_done = True
        ...
        if goal:
            ...
            if self.additive_goals:
                ...
                if not self.goal_range is None:
                    num_attempts = 0
                    while not self.goal_range.contains(self.goal) and num_attempts < 100:
                        self.goal = self.np_random.uniform(low=-self.delta_max, high=self.delta_max, size=self.env.SKILL_DIM) + self.skill_obs(self.last_env_obs)
                        num_attempts += 1
...
class High(gym.Wrapper):
    # TODO: support goal envs. Definetily possible. Will need to log all actions etc. in info to compute reward along the path.
    ...
    def step(self, action):
        ...
        if hasattr(self.env, "xyz_skill"):
            # I Hate that this hack needs to exist.
