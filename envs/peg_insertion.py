...
class PegInsertSawyer(PegInsert):
    ...
    def __init__(self, relative_grip_pos=True, rand_init=False, valid_area=False, z_skill=False, xy_skill=False, xyz_skill=False, 
        ...
        if self.z_skill: # We also modify the observation space to make it include the clip range for the z skill
            self.observation_space['observation'].low[self.AGENT_DIM] = -self.peg_length # first component of the skill space is now the z coordinate.
            self.observation_space['observation'].high[self.AGENT_DIM] = self.peg_length
        if self.xy_skill:
            self.observation_space['observation'].low[self.AGENT_DIM:self.AGENT_DIM+2] = -self.peg_length
            self.observation_space['observation'].high[self.AGENT_DIM:self.AGENT_DIM+2] = self.peg_length
        if self.xyz_skill:
            self.observation_space['observation'].low[self.AGENT_DIM:self.AGENT_DIM+3] = -self.peg_length
            self.observation_space['observation'].high[self.AGENT_DIM:self.AGENT_DIM+3] = self.peg_length
        '''
        # Modify for the non goal-env version.
        if self.z_skill: # We also modify the observation space to make it include the clip range for the z skill
    ...
    def _get_obs(self):
        ...
        return {
            ...
            'achieved_goal' : ref_pt,
            'desired_goal' : self.sim.data.get_site_xpos('target'),
            }
        '''
...
class Insert_XArm7Pos(PegInsertSawyer):
    FRAME_SKIP = 1
    NSUBSTEPS = 20
    # NOTE: Because of the way the peg is mounted, this is really like a 6Dof Sawyer
    # ASSET = "xarm/xarm7_table_pos.xml"
    ASSET = "xarm/kitchen.xml"
    JOINT_RANDOMIZATION_LOW = [-0.5, -1.25, -1.0, 0.0, -1.0, -0.5, -0.5]
    JOINT_RANDOMIZATION_HIGH = [0.5, 0.25, 1.0, 2.0, 1.0, 0.75, 0.5]
    AGENT_DIM = 7*2 + 6
    NUM_JOINTS = 7

class Insert_XArm7PosScale75(PegInsertSawyer):
    # Scale Arena by scaling in first two positions
    FRAME_SKIP = 1
    NSUBSTEPS = 20
    VISUALIZE = True
    # NOTE: Because of the way the peg is mounted, this is really like a 6Dof Sawyer
    ASSET = "xarm/xarm7_table_pos_scale_75.xml"
    JOINT_RANDOMIZATION_LOW = [-0.5, -1.25, -1.0, 0.0, -1.0, -0.5, -0.5]
    JOINT_RANDOMIZATION_HIGH = [0.5, 0.25, 1.0, 2.0, 1.0, 0.75, 0.5]
    AGENT_DIM = 7*2 + 6
    NUM_JOINTS = 7
    ...
    def _get_obs(self):
        ref_pt = self.get_site_com('ref_pt').copy()
        peg_top = self.get_site_com('peg_top').copy() - ref_pt #NOTE: Length of the peg is 0.09 (between the two sites)
        grip_base = self.get_site_com('grip_base').copy()
        if self.relative_grip_pos:
            grip_base = grip_base - ref_pt

        ref_pt /= self.SCALING
        peg_top /= self.SCALING
        grip_base /= self.SCALING
        '''
        # OLD
        if not self.relative_grip_pos:
            grip_base[:2] /= self.SCALING
        ref_pt[:2] /= self.SCALING
        # The peg is the SAME SIZE
        # This means that the relative difference in peg top to bottom is unchanged.
        # SO WE DON'T DO THIS: peg_top[:2] /= self.SCALING
        '''
    ...
    def _get_target_pos(self):
        return self.sim.data.get_site_xpos('target').copy() / self.SCALING

    def display_skill(self, skill):
        # The skill is going to be in the scaled UP space. Need to mult. by SCALING
        if self.z_skill:
            self.model.body_pos[-1][:3] = skill[1:4]
        elif self.xy_skill:
            x, y = skill[0], skill[1]
            if x**2 + y**2 < self.peg_length**2:
                ...
                quat /= np.linalg.norm(quat)                
                self.model.body_quat[-1][:] = quat
            pos_skill = skill[2:5].copy()
            pos_skill *= self.SCALING
            self.model.body_pos[-1][:3] = pos_skill
        elif self.xyz_skill:
            x, y = skill[0], skill[1]            
            if x**2 + y**2 < self.peg_length**2:
                perp_vec = np.cross(np.array([0,0,1]), np.array([x,y,0]))
                ...
                quat /= np.linalg.norm(quat)                
                self.model.body_quat[-1][:] = quat
            pos_skill = skill[3:6].copy()
            pos_skill *= self.SCALING
            self.model.body_pos[-1][:3] = pos_skill
        else:
            pos_skill = skill[:3].copy()
            pos_skill *= self.SCALING
            self.model.body_pos[-1][:3] = pos_skill
