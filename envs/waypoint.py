...
class Waypoint_XArm7Vel(WaypointSawyer):
    ARENA_SPACE_LOW = np.array([0.15, -0.28, -0.05])
    ARENA_SPACE_HIGH = np.array([0.7, 0.28, 0.25])
    FRAME_SKIP = 1
    NSUBSTEPS = 1
    VISUALIZE = True
    # NOTE: Because of the way the peg is mounted, this is really like a 6Dof Sawyer
    ASSET = "xarm/xarm7_table_vel.xml"
    JOINT_RANDOMIZATION_LOW = [-0.5, -1.25, -1.0, 0.0, -1.0, -0.5, -0.5]
    JOINT_RANDOMIZATION_HIGH = [0.5, 0.25, 1.0, 2.0, 1.0, 0.75, 0.5]
    AGENT_DIM = 7*2 + 6
    NUM_JOINTS = 7

class Waypoint_XArm7Pos(WaypointSawyer):
    ARENA_SPACE_LOW = np.array([0.15, -0.28, -0.05])
    ARENA_SPACE_HIGH = np.array([0.7, 0.28, 0.25])
    FRAME_SKIP = 1
    NSUBSTEPS = 20
...
class Waypoint_XArm7PosScale75(WaypointSawyer):
    ...
    def reset(self):
        ...
        self.waypoint = self.np_random.uniform(low=self.ARENA_SPACE_LOW, high=self.ARENA_SPACE_HIGH)
        if self.VISUALIZE:
            waypoint_scale = self.waypoint.copy()
            waypoint_scale *= self.SCALING
            self.model.body_pos[-2][:self.WAYPOINT_DIM] = waypoint_scale
        if self.rand_init:
            qpos = self.init_qpos + self.np_random.uniform(size=self.model.nq, low=self.JOINT_RANDOMIZATION_LOW, high=self.JOINT_RANDOMIZATION_HIGH)
            qvel = self.init_qvel + self.np_random.randn(self.model.nv) * .05
            self.set_state(qpos, qvel)
